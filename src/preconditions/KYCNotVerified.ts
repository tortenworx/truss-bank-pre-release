...
export class KYCNotVerified extends Precondition {
    ...
    private async checkIfUserIsKYC(userId: string): Precondition.AsyncResult {
        ...
        return data ? this.error({ identifier: "Already registered", message: "You already have registed an account with TRUSS Bank! If you believe that this is a mistkake, please open a support ticket." }) : this.ok()
    }
}