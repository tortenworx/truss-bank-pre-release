...
export class QuikDep extends Command {
    ...
    public async contextMenuRun(interaction: ContextMenuCommandInteraction, context: ContextMenuCommand.RunContext): Promise<unknown> {
        ...
        if (interaction.isMessageContextMenuCommand()) {
            ...
            if (Date.now() - message.createdAt.getTime() > 3600000) return await interaction.editReply({ components: [invalid.addTextDisplayComponents(new TextDisplayBuilder().setContent("PostmanPat transactions that are three hours old are not accepted. Please do /deposit instead."))], flags: ["IsComponentsV2"] });
